...
![需要代理](https://www.tensorflow.org/images/tfdbg_screenshot_run_start.png)
...
![需要代理](https://www.tensorflow.org/images/tfdbg_screenshot_run_end_accuracy.png)
...
* 第二个问题，``hidden/Wx_plus_b/MatMuls:0`` 是怎么和代码对应起来的
    ...
    ```
    这段代码中，V1/V2/V3 对应的 Tensor 的名字分别为：``Any_Scope/First_Scope/First`` 、
    ``Any_Scope/Second_Scope/Variable``、 ``Any_Scope/Second_Scope/Variable_1``
    可以看出，如果不指定 ``name 参数`` 则默认为 Tensor/Option 的类名，如果相同 Scope 中有相同 name 的 Tensor/Option 则自动为名称添加序号
