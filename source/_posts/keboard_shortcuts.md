...
改变文本
删除至行尾 (及 N - 1 后续行)，并进入插入模式: ``C``

改变文本
删除 N 个字符，并进入插入模式: ``s``

改变文本
在可视列块模式下: 用键入的文本修改选中文本的每一行: ``{visual}c``
在可视列块模式下: 用键入的文本修改选中各行直至行末: ``{visual}C``
翻转 N 个字符的大小写并前进光标: ``~``
翻转高亮文本的大小写: ``{visual}~``
改高亮的文本为小写: ``{visual}u``
改高亮的文本为大写: ``{visual}U``
...
Ack	暂时不知道 ACK 怎么指定搜索路径，可以在Vim 中通过 cd 命令进入指定目录再搜索	　
Ag	快速搜索		，g
Ag	选取模式下，搜索选中的内容		gv
